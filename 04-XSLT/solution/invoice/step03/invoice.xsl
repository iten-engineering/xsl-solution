...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
      <html>
        ...
        <body>
          ...
          <p>
            <xsl:apply-templates select="invoice/positions" mode="all"/>
          </p>
        </body>
      </html>
    </xsl:template>
    ...
    <!-- 
      template: positions (mode:all)
    -->    
    <xsl:template match="positions" mode="all">
      <table border="1" cellspacing="0" cellpadding="2">
        <tr>
          <th>Pos</th>
          <th>Artikel</th>
        ...
        <xsl:apply-templates select="position" mode="all">
          <xsl:sort select="@pos" data-type="number"/>
        </xsl:apply-templates>
      </table>      
    </xsl:template>
    <!-- 
      template: position
    -->
    <xsl:template match="position" mode="all">
      <xsl:if test="position() mod 2 = 0">
        <xsl:call-template name="position-row-all">
          <xsl:with-param name="class">even</xsl:with-param>
        </xsl:call-template>
      </xsl:if>
      <xsl:if test="not(position() mod 2 = 0)">
    ...
    <!-- 
      template: position-row-all
    -->
    <xsl:template name="position-row-all">
      <xsl:param name="class"/>
      <xsl:variable name="format">#&apos;##0.00</xsl:variable>
      <tr>
        <td class="{$class}"><xsl:value-of select="@pos"/></td>
        <td class="{$class}"><xsl:value-of select="description"/></td>
        <xsl:choose>
          <xsl:when test="@currency = $chf">
            <td class="{$class}"><xsl:value-of select='format-number(@price,"#&apos;###.00","ch")'/></td>        
            <td class="{$class}"><xsl:value-of select='format-number(@price div 1.52,"#&apos;###.00","ch")'/></td>            
          </xsl:when>
          <xsl:when test="@currency = $euro">
            <td class="{$class}"><xsl:value-of select='format-number(@price * 1.52,"#&apos;###.00","ch")'/></td>            
            <td class="{$class}"><xsl:value-of select='format-number(@price,"#&apos;###.00","ch")'/></td>        
          </xsl:when>
          <xsl:otherwise>
            <td class="{$class}">NAV</td>            
            <td class="{$class}">NAV</td>        
          </xsl:otherwise>           
        </xsl:choose>
      </tr>
